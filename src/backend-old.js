...
const getChapterName = async (chapterNumber) => {
    ...
    try {
        const urlResponse = await fetch(`${API_BASE_URL}/chapters/${chapterNumber}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json'
            }
...
const getNumberVerses = async (chapterNumber) => {
    try {
        const response = await fetch(`${API_BASE_URL}/chapters/${chapterNumber}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json'
            }
...
const getVerseText = async (chapterNumber, verseNumber) => {
    try {
        ...
        const data = await response.json();
        // NOTE: this may be very ineffienct
        data.verses = data.verses.filter(verse => verse.verse_key == `${chapterNumber}:${verseNumber}`);
...
const getReciters = async () => {
    try {
        const response = await fetch(`${API_BASE_URL}/v4/resources/recitations`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json'
            }
